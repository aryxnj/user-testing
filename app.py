...
def scroll_to_top(counter):
    components.html(
        ...
        """,
        height=0,  # Minimal height to avoid affecting layout
        width=0,
        key=f'scroll_script_{counter}'  # Unique key based on counter
    )
...
# Instructions Page Function Call
def instructions_page():
    st.title("📋 Testing Protocol Instructions 📋")
    st.markdown("""
        ### Scoring Method
...
# Define other page functions similarly...
# For brevity, they're omitted here since they've been defined above
...
# Inject JavaScript to scroll to top after content is rendered
scroll_to_top(st.session_state.scroll_counter)
